...
export class FilterCoursesComponent {
  ...
  getNewTotal(COURSE: any, DESCOUNT_BANNER: any) {
    if (DESCOUNT_BANNER.type_discount == 1) {
      return (
        COURSE.precio_desc -
        COURSE.precio_desc * (DESCOUNT_BANNER.discount * 0.01)
      );
    } else {
      return COURSE.precio_desc - DESCOUNT_BANNER.discount;
    }
  }
  ...
  addCart(LANDING_COURSE: any, DESCOUNT_CAMPAING: any = null) {
    ...
    let data = {
      ...
      type_discount: LANDING_COURSE.discount_g
        ? LANDING_COURSE.discount_g.type_discount
        : null,
      discount: LANDING_COURSE.discount_g
        ? LANDING_COURSE.discount_g.discount
        : null,
      type_campaing: LANDING_COURSE.discount_g
        ? LANDING_COURSE.discount_g.type_campaing
        : null,
      code_cupon: null,
      code_discount: LANDING_COURSE.discount_g
        ? LANDING_COURSE.discount_g.code
        : null,
      precio_unitario: LANDING_COURSE.precio_desc,
  ...
  addCategorie(ID_CATEGORIE: any) {
    let INDEX = this.selected_categories.findIndex(
      (item: any) => ID_CATEGORIE == item
    );
    if (INDEX != -1) {
  ...
  addInstructores(INSTRUCT: any) {
    let INDEX = this.instructores_selected.findIndex(
      (item: any) => INSTRUCT.id == item
    );
    if (INDEX != -1) {
