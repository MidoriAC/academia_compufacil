...
export class ProfileClientComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tiendaAuth.profileClient().subscribe((resp: any) => {
      console.log('Enrolled Courses:', resp.enrolled_courses);
  ...
  saveReview() {
    ...
    if (this.sale_detail_selected.review) {
      this.tiendaAuth
        .updateReview(data, this.sale_detail_selected.review.id)
        .subscribe((resp: any) => {
          console.log(resp);
          alertSuccess('LA RESEÑA SE EDITO CORRECTAMENTE');
          let INDEX = this.sale_details.findIndex(
            (item: any) => item.id == this.sale_detail_selected.id
          );
          if (INDEX != -1) {
  ...
  //   this.tiendaAuth.downloadCertificate(courseId).subscribe(
  //     (data: Blob) => {
  ...
  //     },
  //     (error) => {
  //       alertDanger('No se pudo descargar el certificado');
  //     }
  //   );
  // }
  downloadCertificate(courseId: number) {
    const userId = this.authService.user.id;
    this.tiendaAuth.downloadCertificate(userId, courseId).subscribe(
      (response: any) => {
        ...
        link.click();
        window.URL.revokeObjectURL(url);
        alertSuccess(['Certificado descargado exitosamente']);
      },
      (error) => {
        console.error('Error downloading certificate:', error);
        if (error.status === 404) {
          alertDanger(['Certificado no encontrado']);
        } else {
          alertDanger([
            'Error al descargar el certificado: ' + error.statusText,
          ]);
        }
      }
    );
